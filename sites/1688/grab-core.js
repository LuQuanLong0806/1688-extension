...
.logo{display:inline-flex;align-items:center;gap:3px;margin-right:20px;font-size:26px;font-weight:600;letter-spacing:1px;color:#ff6a00;filter:drop-shadow(1px 2px 4px rgba(76,175,80,.25));animation:logoIn .8s cubic-bezier(.34,1.56,.64,1) both,logoFloat 3s ease-in-out .8s infinite}
.logo svg{width:42px;height:46px;flex-shrink:0}
...
<div class="tb">
  ...
  <button id="btnCopy" class="s3" style="display:none">📋 复制选中地址</button>
  ...
  function setupCopyIcons() {
    ...
    if (skuEl) {
      ...
      if (!s) {
        ...
        s.textContent = '#skuSelection .label-name{overflow:visible!important;display:inline-flex!important;align-items:center!important;gap:2px!important}#productAttributes table td:nth-child(even),#productPackInfo table td{position:relative;overflow:visible!important}';
