...
router.post('/category-mappings', (req, res) => {
  ...
  try {
    ...
    if (!existing) {
      ...
      if (cloudDb.connected) {
        cloudDb.cloudRun('INSERT OR IGNORE INTO category_mappings (category_name, custom_category, count, source, created_at, updated_at) VALUES (?, ?, 1, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)', [categoryName, customCategory, 'manual']).catch(function () {});
      }
    }
    res.json({ ok: true });
  } catch (e) {
...
router.delete('/keyword-rels/:id', (req, res) => {
  ...
  run('UPDATE keyword_category_rel SET valid = 0, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [id]);
  if (cloudDb.connected && rel) {
    cloudDb.cloudRun('UPDATE keyword_category_rel SET valid = 0, updated_at = CURRENT_TIMESTAMP WHERE keyword = ? AND category_name = ?', [rel.keyword, rel.category_name]).catch(function () {});
  }
  res.json({ ok: true });
});
...
router.post('/keyword-rels/batch-invalidate', (req, res) => {
  ...
  ids.forEach(id => {
    ...
    run('UPDATE keyword_category_rel SET valid = 0, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [parseInt(id)]);
    if (cloudDb.connected && rel) {
      cloudDb.cloudRun('UPDATE keyword_category_rel SET valid = 0, updated_at = CURRENT_TIMESTAMP WHERE keyword = ? AND category_name = ?', [rel.keyword, rel.category_name]).catch(function () {});
    }
  });
...
router.post('/keyword-synonyms', (req, res) => {
  ...
  try {
    ...
    if (cloudDb.connected) {
      cloudDb.cloudRun('INSERT OR IGNORE INTO keyword_synonyms (word_a, word_b, created_at, updated_at) VALUES (?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)', [wordA, wordB]).catch(function () {});
    }
    res.json({ ok: true });
  } catch (e) {
    res.status(500).json({ error: e.message });
