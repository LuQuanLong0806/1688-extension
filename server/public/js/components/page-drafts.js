...
Vue.component('page-drafts', {
  ...
  methods: {
    ...
    batchStage: function (targetStage) {
      ...
      var label = targetStage === 'ready' ? '移入待发布' : '退回未处理';
      vm.$Modal.confirm({
        title: '批量操作',
    ...
    <div class="automation-page">
      ...
      <div class="action-bar">
        <div style="display:flex;gap:8px">
          <i-button v-if="selectedIds.length" type="primary" size="small" @click="batchStage('ready')" :loading="batchOperating">
            批量移入待发布
          </i-button>
          <i-button v-if="selectedIds.length" type="default" size="small" @click="batchStage('none')" :loading="batchOperating">
            批量退回未处理
          </i-button>
        ...
        <i-table :columns="columns" :data="list" :loading="loading" stripe @on-selection-change="onSelectionChange" style="margin-bottom:0;">
          ...
          <template slot="actions" slot-scope="{ row }">
            <div class="action-btns">
              ...
              <Button type="success" size="small" @click="setStage(row, 'ready')">移入待发布</Button>
              <Button type="default" size="small" @click="setStage(row, 'none')">退回</Button>
            </div>
          </template>
        </i-table>
      </div>
