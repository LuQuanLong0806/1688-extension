...
Vue.component('page-products', {
  ...
  created: function () {
    ...
    this.columns = [
      ...
      {
        title: '预览', width: 64, align: 'center', ellipsis: false, className: 'col-thumb',
        render: function (h, params) {
          ...
          return h('img', {
            ...
            class: 'cell-thumb',
            on: {
              mouseenter: function (e) { vm.$root.$refs.thumbPreview.open(img, e); },
              mousemove: function (e) { vm.$root.$refs.thumbPreview.move(e); },
              mouseleave: function () { vm.$root.$refs.thumbPreview.close(); }
            }
          });
        }
      },
      { title: '标题', key: 'title', width: 200, ellipsis: false, render: function (h, params) {
        return h('span', { class: 'title-text' }, params.row.title || '-');
      }},
      { title: 'SKU', minWidth: 180, render: function (h, params) {
        var skus = JSON.parse(params.row.skus || '[]');
        ...
        var text = names.join('、');
        return h('span', {
          style: { fontSize: '12px', lineHeight: '1.4', display: '-webkit-box', WebkitLineClamp: '2', WebkitBoxOrient: 'vertical', overflow: 'hidden', wordBreak: 'break-all' }
        }, text);
      }},
      { title: '来源地址', width: 180, render: function (h, params) {
        var url = params.row.source_url;
        if (!url) return h('span', { style: { color: '#ccc' } }, '-');
        var short = url.length > 40 ? url.substring(0, 40) + '...' : url;
        return h('a', {
          attrs: { href: url, target: '_blank', title: url },
          style: { fontSize: '12px', color: '#ff6a00', wordBreak: 'break-all', lineHeight: '1.4', display: '-webkit-box', WebkitLineClamp: '2', WebkitBoxOrient: 'vertical', overflow: 'hidden' }
        }, short);
      }},
      { title: '状态', width: 90, align: 'center', render: function (h, params) {
        var s = params.row.status;
        return h('span', { class: 'status-dot ' + (s === 0 ? 'unused' : 'used') },
          s === 0 ? '未使用' : '已使用');
      }},
      { title: '采集时间', key: 'created_at', width: 160 },
      {
        title: '操作', width: 140, align: 'center', className: 'col-actions',
        render: function (h, params) {
          var row = params.row;
          return h('div', { class: 'action-btns' }, [
            h('Tooltip', { props: { content: '查看详情', placement: 'top', transfer: true } }, [
              h('Button', {
                props: { type: 'primary', size: 'small', icon: 'ios-eye' },
                on: { click: function () { vm.$root.openDetail(row.id); } }
              })
            ]),
            h('Tooltip', { props: { content: '引用打开', placement: 'top', transfer: true } }, [
              h('Button', {
                props: { size: 'small', icon: 'ios-link' },
                on: { click: function () { vm.openSource(row.source_url); } }
              })
            ]),
            h('Tooltip', { props: { content: '新建打开', placement: 'top', transfer: true } }, [
              h('Button', {
                props: { size: 'small', icon: 'md-open' },
                on: { click: function () { vm.openSource(row.source_url); } }
              })
            ]),
            h('Tooltip', { props: { content: '删除', placement: 'top', transfer: true } }, [
              h('Button', {
                props: { type: 'error', size: 'small', icon: 'ios-trash' },
                on: { click: function () { vm.deleteProduct(row.id); } }
              })
            ])
          ]);
        }
  ...
  mounted: function () { this.loadList(1); },
  methods: {
    loadList: function (p) {
      var vm = this;
      if (p) vm.page = p;
      ...
      fetch('/api/product?' + params.toString())
        ...
        .then(function (d) { vm.list = d.list; vm.total = d.total; vm.loading = false; })
    ...
    onSelectionChange: function (sel) { this.selectedIds = sel.map(function (i) { return i.id; }); },
    openSource: function (url) {
      if (url) window.open(url, '_blank');
    },
    deleteProduct: function (id) {
      var vm = this;
      this.$Modal.confirm({ title: '确认删除', content: '确认删除此商品？',
        onOk: function () {
          fetch('/api/product/' + id, { method: 'DELETE' })
            .then(function () { vm.loadList(); vm.$root.loadStats(); });
        }
      });
    },
    batchDelete: function () {
      ...
      this.$Modal.confirm({ title: '确认删除', content: '确认删除 ' + vm.selectedIds.length + ' 条商品？',
        onOk: function () {
          fetch('/api/product/batch-delete', {
            method: 'POST', headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ ids: vm.selectedIds })
          }).then(function () { vm.selectedIds = []; vm.loadList(); vm.$root.loadStats(); });
        }
      });
    }
  },
