...
### 3.13 店小秘信息区域 `#dxmInfo`

| 选择器 | 含义 | 使用场景 |
|--------|------|---------|
| `#dxmInfo` | 店小秘信息卡片（仅店小秘系统生效） | 自动填表: 定位来源URL区域 |
| `#dxmInfo input[name="sourceUrl"]` | 来源URL输入框（maxlength=1000） | 自动填表: 填入来源URL（`data.source_url`） |
| `#dxmInfo form .ant-form-item-label label` (文字含"来源URL") | 来源URL标签 | 自动填表: 确认来源区域 |
| `#dxmInfo form .ant-form-item-label label` (文字含"店小秘分类") | 店小秘分类标签 | 自动填表: 分类相关 |
...
| `loadAutoFill()` / `saveAutoFill(val)` | 自动填表开关 |
| `loadFromServer()` | 从服务器加载配置并同步到 localStorage |
| `syncToServer(key, value)` | 防抖300ms批量同步配置到服务器 |
...
**智能属性复用策略**:
- **已有属性 >= 需要属性数**: 取消所有勾选 → 编辑前 N 个已有属性（改名）→ 勾选 → 多余属性保持取消
- **已有属性 < 需要属性数**: 取消所有勾选 → 编辑所有已有属性（改名）→ 勾选 → 通过添加框动态添加剩余差值
- **无已有属性**: 直接通过添加框添加
- **属性值清洗**: 自动过滤特殊符号（保留中文、英文、数字、空格、括号、横杠、斜杠、小数点），清洗变更时橙色气泡提醒3秒

**属性编辑操作**: 点击 `.btn-edit` → 等待 `.edit-inp` 输入框出现 → `setInputValue` 设新值 → 点击 `.btn-save` 保存
...
### 7.8 自动填表（collectId 参数触发）

**触发条件**: URL 含 `collectId` 参数，且 `BeeConfig.loadAutoFill()` 开关开启（默认关闭）。
...
| 4 | 加载描述图 | 存储到全局 `window.__autoFillDescImages` | 供描述按钮使用 |
...
**目的**: 将采集数据的来源URL填入店小秘信息区的来源URL输入框。
**前置**: 页面已加载，`#dxmInfo` 区域可见。

```javascript
var input = document.querySelector('#dxmInfo input[name="sourceUrl"]');
if (input) {
  input.focus();
  Config.setInputValue(input, sourceUrl);
  input.blur();
}
```
...
**目的**: 复用已有变种属性（编辑改名+勾选），避免重复添加报错。
**前置**: SKU属性区域 `#skuAttrsInfo` 已渲染。

```javascript
// 策略:
// 已有 >= 需要: 编辑前N个 + 勾选, 多余取消
// 已有 < 需要:  编辑全部已有 + 勾选, 动态添加剩余
// 无已有:       全部动态添加
...
setTimeout(function () {
  ...
  Config.setInputValue(input, targetValues[i]);
  setTimeout(function () {
    label.querySelector('.btn-save').click();
    // 勾选此属性
...
**属性值清洗**: 自动过滤特殊符号（正则 `/[^一-龥a-zA-Z0-9\s()（）\-\/\\.·]/g`），有变更时橙色气泡提醒。
...
| 字段 | 类型 | 来源选择器 | 说明 |
|------|------|-----------|------|
| `sourceUrl` | string | `location.href` | 商品来源URL |
| `title` | string | `#productTitle h1` | 商品标题 |
| `category` | object | HTML 正则匹配 | 类目信息（catId/leafCategoryId/topCategoryId 等） |
| `mainImages` | string[] | `#gallery .od-gallery-preview img.preview-img` | 主图（最多8张） |
| `descImages` | string[] | `#description img` + iframe 内 img | 描述图 |
| `detailImages` | string[] | `#description .od-collapse-module .collapse-body v-detail-q` Shadow DOM 内 img | 详情图（Shadow DOM） |
| `attrs` | string[] | `.sku-item-name` | SKU属性名列表 |
| `skus` | object[] | SKU列表弹窗表格 (`.ap-platformSKUs-table`) | SKU数据（image/sku/price/name/dimensions/weight） |

### 11.2 采集流程

1. 检查是否已采集（`GET /api/product/check?offerId=`）
2. 滚动加载页面（确保懒加载图片全部加载）
3. 采集数据:
   - 主图: 从 `#gallery .od-gallery-preview` 提取 `img.preview-img`
   - 描述图: 从 `#description` 及 iframe 提取所有 `img`
   - 详情图: 从 `v-detail-q` Shadow DOM 提取所有 `img`
   - SKU: 点击"SKU列表"按钮（**5s轮询重试**，超时回退DOM提取）→ 解析弹窗表格 → 关闭弹窗
   - 包装信息: 从 `#productPackInfo table` 提取尺寸/重量
4. 保存到服务器（`POST /api/product`）

### 11.3 SKU列表按钮重试机制

SKU列表按钮（`.ap-pdptoolbar__item-text` 文字为"SKU列表"）可能延迟加载，采用 `requestAnimationFrame` 轮询查找，5s超时后回退到 DOM 直接提取属性名作为 SKU。
...
| 接口 | 方法 | 说明 |
|------|------|------|
| `/api/product` | POST | 新增商品（含 detailImages 字段） |
| `/api/product/:id` | GET | 获取详情（返回 main_images/desc_images/detail_images/skus 等） |
| `/api/product/:id` | PUT | 更新商品（支持 detailImages 等字段） |
| `/api/product` | GET | 列表查询（分页/搜索/筛选） |
| `/api/product/check` | GET | 检查是否已采集 |
| `/api/product/latest` | GET | 获取最新一条 |

**detail_images 字段**: 通过 `ALTER TABLE` 增量添加到 products 表，JSON 数组格式，与 main_images/desc_images 一致。
