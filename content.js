(function () {
  ...
  [
    ...
    try {
      document.querySelectorAll(sel).forEach(getAttrs);
    } catch (e) {}
  });
  ...
  document
    ...
    .forEach(function (el) {
      var p =
        el.getAttribute('data-poster') || el.getAttribute('data-video-poster');
      if (p) addImage(p);
    });
  ...
  h +=
    '.hd h1{font-size:24px;margin-bottom:8px}.hd p{opacity:.9;font-size:14px}';
  h +=
  ...
  h +=
    '.tb button:hover{background:#ff8533}.tb button.s2{background:#666}.tb button.s2:hover{background:#888}';
  h +=
    '.tb button.s3{background:#ff4444}.tb button.s3:hover{background:#ff6666}';
  h += '.tb button.s4{background:#888}.tb button.s4:hover{background:#aaa}';
  h +=
    '.cnt{color:#666;font-size:14px;margin-left:auto}.cnt b{color:#ff6a00;font-size:18px}';
  h +=
  ...
  h +=
    '.card .iw{width:100%;height:200px;background:#fafafa;display:flex;align-items:center;justify-content:center;overflow:hidden;pointer-events:none}';
  ...
  h +=
    '.card .ck input{width:18px;height:18px;accent-color:#ff6a00;cursor:pointer;pointer-events:auto}';
  h +=
  ...
  h +=
    '.card .url{word-break:break-all;font-size:11px;color:#666;max-height:48px;overflow:hidden}';
  h += '.card .ac{margin-top:8px;display:flex;gap:8px}';
  h +=
    '.card .ac a,.card .ac button{font-size:12px;color:#ff6a00;border:1px solid #ff6a00;border-radius:3px;background:none;cursor:pointer;padding:2px 8px;text-decoration:none;pointer-events:auto}';
  h +=
    '.card .ac a:hover,.card .ac button:hover{background:#ff6a00;color:#fff}';
  h +=
    '.po{display:none;position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,.85);z-index:9999;justify-content:center;align-items:center;cursor:pointer}';
  h +=
    '.po.show{display:flex}.po img{max-width:90%;max-height:90%;border-radius:8px}';
  h +=
  ...
  h +=
    ...
    '</b></p></div>';
  h +=
    '<div class="tb"><button id="btnAll">✅ 全选</button><button id="btnNone" class="s2">⬜ 取消全选</button>';
  h +=
    '<button id="btnCopy">📋 复制选中地址</button><button id="btnDl" class="s3">💾 下载列表</button>';
  h += '<button id="btnSmall" class="s4">🔍 显示小图</button>';
  h += '<div class="cnt">已选 <b id="sc">0</b> 张</div></div>';
  h +=
    '<div class="grid" id="grid"></div><div class="po" id="po"><img id="pi"></div>';

  h += '<scr' + 'ipt>';
  h += 'var urls=' + urlsJson + ';var grid=document.getElementById("grid");';
  h +=
    'var scEl=document.getElementById("sc");var poEl=document.getElementById("po");var piEl=document.getElementById("pi");';
  h += 'var toastEl=document.getElementById("toast");';
  h +=
    'function esc(s){return s.replace(/&/g,"\\x26amp;").replace(/</g,"\\x26lt;").replace(/>/g,"\\x26gt;").replace(/"/g,"\\x26quot;");}';
  h +=
  ...
  h +=
    'd.innerHTML=\'<div class="ck"><input type="checkbox" class="ci" id="c\'+i+\'"><label for="c\'+i+\'">#\'+(i+1)+\'</label></div>\';';
  h +=
    'd.innerHTML+=\'<div class="iw"><img src="\'+esc(u)+\'" onerror="this.style.opacity=0.3" onload="fImg(this)"></div>\';';
  h +=
    'd.innerHTML+=\'<div class="ua"><div class="url">\'+esc(u)+\'</div><div class="ac"><a href="\'+esc(u)+\'" target="_blank">打开</a><button class="cpbtn" data-u="\'+esc(u)+\'">复制</button></div></div>\';';
  ...
  h +=
    'if(e.target.tagName==="IMG"&&e.target.closest(".iw")){piEl.src=e.target.src;poEl.classList.add("show");return;}';
  ...
  h +=
    'document.querySelectorAll(".ci").forEach(function(c){c.checked=true;c.closest(".card").classList.add("on");});updateCount();});';
  ...
  h +=
    'poEl.addEventListener("click",function(){poEl.classList.remove("show");});';
