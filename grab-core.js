(function () {
  ...
  function generateResultPage(images, groups) {
    ...
    h += '.preview .toolbar{position:fixed;bottom:24px;left:50%;transform:translateX(-50%);display:flex;gap:10px;background:rgba(0,0,0,.6);padding:8px 16px;border-radius:24px;z-index:99999}';
    h += '.preview .toolbar button{width:40px;height:40px;border-radius:50%;border:none;background:rgba(255,255,255,.2);color:#fff;font-size:18px;cursor:pointer;display:flex;align-items:center;justify-content:center}';
...
h += '<button id="btnSmall" class="s4">\uD83D\uDD0D \u663E\u793A\u5C0F\u56FE</button>';
    ...
    h += '<div class="grid" id="grid"></div><div class="preview" id="preview"><div class="close" id="previewClose">\u2715</div><img id="previewImg"><div class="toolbar" id="pvToolbar"><button id="pvZoomIn" title="\u653E\u5927">+</button><button id="pvZoomOut" title="\u7F29\u5C0F">-</button><button id="pvRotL" title="\u5DE6\u65CB">\u21BA</button><button id="pvRotR" title="\u53F3\u65CB">\u21BB</button><button id="pvReset" title="\u91CD\u7F6E">\u21BA\u21BB</button><button id="pvCopy" title="\u590D\u5236\u56FE\u7247\u5730\u5740">\uD83D\uDCCB</button></div></div>';
    h += '<scr' + 'ipt>';
...
h += 'var pvEl=document.getElementById("preview");var pvImg=document.getElementById("previewImg");';
    h += 'var _pz=1,_px=0,_py=0,_pr=0,_pd=false,_psx=0,_psy=0;';
    ...
    h += 'document.getElementById("pvCopy").addEventListener("click",function(e){e.stopPropagation();copyOne(pvImg.src);});';
