(function () {
  ...
  function generateResultPage(images, groups) {
    ...
    h += '<button id="btnSmall" class="s4">\uD83D\uDD0D \u663E\u793A\u5C0F\u56FE</button><button id="btnSort" class="s4">\uD83D\uDD39 \u5C3A\u5BF8\u6392\u5E8F</button>';
    ...
    h += 'var openLink=document.createElement("a");openLink.href=u;openLink.target="_blank";openLink.textContent="\u6253\u5F00";';
    h += 'var cpBtn=document.createElement("button");cpBtn.className="cpbtn";cpBtn.textContent="\u590D\u5236";cpBtn.dataset.u=u;';
    ...
    h += 'var _fd=0,_ft=urls.length,_sn=0,_sorted=false;';
    h += 'function fImg(el){_fd++;';
    h += 'el.closest(".card")._area=el.naturalWidth*el.naturalHeight;';
    h += 'if(el.naturalWidth<200&&el.naturalHeight<200){var c=el.closest(".card");c.classList.add("small");c.style.display="none";_sn++;}';
    h += 'if(_fd===_ft){sortBySize();var n=grid.querySelectorAll(".card").length;var p=document.querySelector(".hd p");if(p)p.innerHTML="\u6709\u6548\u56FE\u7247 <b>"+(n-_sn)+"</b> \u5F20 | \u5C0F\u56FE <b>"+_sn+"</b> \u5F20";}}';
    h += 'function sortBySize(){';
    h += 'var cards=Array.from(grid.children);cards.sort(function(a,b){return (b._area||0)-(a._area||0);});';
    h += 'cards.forEach(function(c){grid.appendChild(c);});_sorted=true;}';
    ...
    h += 'var _origOrder=null;';
    h += 'document.getElementById("btnSort").addEventListener("click",function(){';
    h += 'if(!_sorted){sortBySize();this.textContent="\uD83D\uDD39 \u539F\u59CB\u987A\u5E8F";_sorted=true;}';
    h += 'else{if(!_origOrder)_origOrder=Array.from(grid.children).sort(function(a,b){return parseInt(a.dataset.idx)-parseInt(b.dataset.idx);});';
    h += '_origOrder.forEach(function(c){grid.appendChild(c);});this.textContent="\uD83D\uDD39 \u5C3A\u5BF8\u6392\u5E8F";_sorted=false;}});';
